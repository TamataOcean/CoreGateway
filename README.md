...
## Troubleshooting
 Updating the system using `sudo apt upgrade` at the very beginning of the installation may update the kernel. If so, this prevents configuration of `iptables` when configuring the access point. `iptables` kernel modules were updated and the system needs to reboot to be able to use them. Therefore, the system update has been moved at the very end of the installation process, right before rebooting.